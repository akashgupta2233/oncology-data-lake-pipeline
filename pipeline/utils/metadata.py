...
def extract_schema(
    dataframe: pd.DataFrame | None = None, file_path: Path | None = None
) -> list[dict[str, str]]:
    ...
    return [
        {"name": column, "dtype": str(dtype)} for column, dtype in frame.dtypes.items()
    ]


def _file_manifest_entry(
    path: Path, zone_root: Path, processing_timestamp: str
) -> dict[str, Any]:
...
def generate_manifest(
    zone_root: Path, *, processing_timestamp: str | None = None
) -> dict[str, Any]:
